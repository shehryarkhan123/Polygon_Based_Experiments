...
def _c_table(history: dict) -> str:
    ...
    for row in history["grid"]:
        mark = " (best)" if row["C"] == best else ""
        rows.append(
            f"| {row['C']}{mark} | {row['val_macro_f1']:.4f} | {row['val_acc']:.4f} |"
        )
    return "\n".join(rows)
...
def write_readme(
    ...
    rel = lambda p: Path(p).relative_to(SVM_DIR).as_posix()
    maj_name, maj_pct = _majority_share(counts["test"])
    maj_n = counts["test"][maj_name]
    test_n = sum(counts["test"].values())
    top_feat = max(zip(perm["bands"], perm["importances_mean"]), key=lambda r: r[1])[0]
    w = weights

    text = f"""# Linear SVM: 5-class land cover (7 features)
...
- C chosen on **validate** only (`{{0.1, 1, 10}}`). Best C = **{history['best_C']}** (val macro F1 {history['best_val_macro_f1']:.4f})
- Test scored **once**
...
Linear SVM is the better match for this data. Leftover mistakes (mainly CPF) are unseen fields, not the wrong kernel.
...
**Problem.** A dummy that always predicts **{maj_name}** would get about **{maj_pct:.1f}%** test accuracy and **0** recall on CPF, SL, water, and road. This is **not** a dummy dataset. It is a fake always-{maj_name} guess on the **real** test pixels ({maj_n:,} / {test_n:,}). If the SVM ignored small classes, overall accuracy could still look high because urban + water are ~67% of pixels.

**Solution.** `LinearSVC(..., class_weight="balanced")`. sklearn sets weight `N / (5 × n_c)` so road (smallest) is up-weighted (**{w['road']:.3f}**) and urban (largest) is down-weighted (**{w['urban']:.3f}**). We report balanced accuracy and macro F1, not only overall accuracy. We did **not** downsample urban/water.
...
![C grid]({rel(figure_paths['c_grid'])})
...
"""
    (SVM_DIR / "SVM_README.md").write_text(text, encoding="utf-8")
    print(f"wrote {SVM_DIR / 'SVM_README.md'}")
