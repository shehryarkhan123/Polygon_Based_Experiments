...
from config import BANDS, CLASS_NAMES, N_FEATURES, RESULTS_DIR, SEED
...
def _drop_table(raw: dict, kept: dict) -> str:
    rows = [
        "| Class | Raw pixels | Kept (full 3×3) | Dropped (edges) | Kept % |",
        "|---|---:|---:|---:|---:|",
    ]
    for name in CLASS_NAMES:
        r = raw["train"][name] + raw["validate"][name] + raw["test"][name]
        k = kept["train"][name] + kept["validate"][name] + kept["test"][name]
        rows.append(f"| {name} | {r:,} | {k:,} | {r - k:,} | {_pct(k, r)} |")
    rtot = sum(
        raw[s][n] for s in ("train", "validate", "test") for n in CLASS_NAMES
    )
    ktot = sum(
        kept[s][n] for s in ("train", "validate", "test") for n in CLASS_NAMES
    )
    rows.append(f"| **All** | **{rtot:,}** | **{ktot:,}** | **{rtot - ktot:,}** | **{_pct(ktot, rtot)}** |")
    return "\n".join(rows)
...
def write_readme(
    ...
    rel = (
        lambda p: Path(p).relative_to(SPATIAL_DIR).as_posix()
    ...
    band_txt = ", ".join(BANDS)
    text = f"""# Spatial-context Transformer: 5-class land cover (3×3, {N_FEATURES} features)
...
- Each sample is a **3×3 patch**: 9 pixels × {N_FEATURES} features (`{band_txt}`)
...
**Problem.** A dummy that always predicts **{maj_name}** would get about **{maj_pct:.1f}%** test accuracy and **0** recall on the other classes. This is **not** a dummy dataset. It is a fake always-{maj_name} guess on the **real** test pixels after the 3×3 filter ({maj_n:,} / {test_n:,}).

**Solution.** Weighted cross-entropy with `w_c = N / (5 × n_c)` on remaining train pixels. Road is up-weighted (**{w['road']:.3f}**); urban is down-weighted (**{w['urban']:.3f}**). We did **not** downsample urban/water.

**Evidence (this run).** Test accuracy is **{test_metrics['accuracy']:.4f}** vs the {maj_pct:.1f}% dummy. Balanced accuracy is **{test_metrics['balanced_accuracy']:.4f}**. Road test recall is **{test_metrics['per_class']['road']['recall']:.4f}** and CPF 253 test recall is **{test_metrics['per_class']['CPF 253']['recall']:.4f}**.
...
One sample is **`(9, {N_FEATURES})`**. Batch shape is **`(batch, 9, {N_FEATURES})`**.

- 9 spatial tokens (3×3). Features per token: `{band_txt}`
- Linear embed {N_FEATURES} → 64. **No** positional encoding
...
Each spectral feature is shuffled on all 9 tokens, {perm['n_samples']:,} validation patches, 10 repeats. **{top_feat}** is strongest this run.
...
"""
    (SPATIAL_DIR / "SPATIAL_README.md").write_text(text, encoding="utf-8")
    print(f"wrote {SPATIAL_DIR / 'SPATIAL_README.md'}")
