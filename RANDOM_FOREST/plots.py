...
def plot_grid(history: dict) -> str:
    grid = history["grid"]
    labels = [
        f"{r['n_estimators']}/{_depth_label(r['max_depth'])}" for r in grid
    ]
    ...
    ax.bar(x - width / 2, f1s, width, label="val macro F1")
    ax.bar(x + width / 2, accs, width, label="val accuracy")
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_xlabel("n_estimators / max_depth")
    ax.set_ylabel("Score")
    ax.set_title("Random Forest grid (validation set)")
    ax.legend()
    return _save(fig, "grid.png")
...
def plot_confusion(metrics: dict, split: str) -> str:
    ...
    ax.set_title(f"RF confusion matrix ({SPLIT_LABEL.get(split, split)})")
...
def plot_roc(metrics: dict, split: str) -> str:
    ...
    ax.set_title(f"RF ROC OvR ({SPLIT_LABEL.get(split, split)}), macro AUC = {metrics['roc_auc']:.3f}")
    ax.legend()
...
def plot_pr(metrics: dict, split: str) -> str:
    ...
    ax.set_title(f"RF precision-recall OvR ({SPLIT_LABEL.get(split, split)})")
    ax.legend()
...
def plot_permutation(perm: dict) -> str:
    ...
    ax.set_title("RF permutation importance (feature accuracy drop)")
...
def plot_gini(gini: dict) -> str:
    fig, ax = plt.subplots(figsize=(7, 4))
    ax.bar(gini["bands"], gini["importances"], color="#F58518")
    ax.set_title("RF Gini importance (mean decrease in impurity)")
    ax.set_xlabel("Feature")
    ax.set_ylabel("Importance")
    ax.tick_params(axis="x", rotation=20)
    return _save(fig, "gini_importance.png")


def plot_all(
    history: dict, val_metrics: dict, test_metrics: dict, perm: dict, gini: dict
) -> dict:
    return {
        "grid": plot_grid(history),
