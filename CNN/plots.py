...
def plot_history(history: dict) -> list[str]:
    ...
    ax.set_title("CNN training and validation accuracy")
    ax.set_xlabel("Epoch")
...
def plot_confusion(metrics: dict, split: str) -> str:
    ...
    ax.set_title(f"Confusion matrix ({SPLIT_LABEL.get(split, split)})")
...
def plot_roc(metrics: dict, split: str) -> str:
    ...
    ax.set_title(f"ROC OvR ({SPLIT_LABEL.get(split, split)}), macro AUC = {metrics['roc_auc']:.3f}")
    ax.legend()
...
def plot_pr(metrics: dict, split: str) -> str:
    ...
    ax.set_title(f"Precision-recall OvR ({SPLIT_LABEL.get(split, split)})")
    ax.legend()
...
def plot_permutation(perm: dict) -> str:
    ...
    ax.set_title("Permutation importance (feature accuracy drop)")
